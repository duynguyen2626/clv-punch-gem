 ...
 * @param {Object} params
 * @param {string} params.photo - Photo URL
 * @param {string} [params.caption] - Caption text (HTML parse mode)
 * @param {Array<Array<{text: string, callback_data: string}>>} [params.buttons] - Inline keyboard rows
 */
async function sendPhoto({ photo, caption = '', buttons = null, chatId: overrideChatId, token: overrideToken } = {}) {
    let token = overrideToken;
    let chatId = overrideChatId;
    ...
    if (!token || !chatId || !photo) return null;
 ...
 */
async function editMessage(token, chatId, messageId, text, buttons = null) {
    try {
        const payload = {
            chat_id: chatId,
...
module.exports = { sendTelegram, sendPhoto, answerCallback, editMessage, registerWebhook };
