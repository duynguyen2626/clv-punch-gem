 ...
 * @param {string[]} dateKeys - Array of YYYY-MM-DD strings
 * @returns {Promise<Array<{date: string, period: string, status: string, punch_time: string|null, created_at: number}>>}
 */
async function getPunchHistory(dateKeys) {
  ...
  const placeholders = dateKeys.map(() => '?').join(',');
  const result = await client.execute({
    sql: `SELECT date, period, status, punch_time, created_at 
          FROM punch_history 
...
async function logSystemEvent(eventType, eventData, userSource = 'api') {
  ...
  const now = Math.floor(Date.now() / 1000);
  await client.execute({
    sql: `INSERT INTO system_events (event_type, event_data, user_source, created_at)
          VALUES (?, ?, ?, ?)`,
...
async function getSystemEvents(eventTypes = null, limit = 100) {
 try {
    ...
    if (typeof client.execute !== 'function') {
      console.warn('[getSystemEvents] client.execute is not a function, returning empty array');
      return [];
    }
    ...
    return result.rows.map(row => ({
      ...
      event_data: JSON.parse(/** @type {string} */ (row.event_data)),
      user_source: /** @type {string} */ (row.user_source),
      created_at: /** @type {number} */ (row.created_at),
    }));
  } catch (err) {
    console.warn('[getSystemEvents] Error:', err.message);
    return [];
  }
}
